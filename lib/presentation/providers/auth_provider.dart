...
class AuthNotifier extends StateNotifier<LoginState> {
  ...
  AuthNotifier(this._authDataSource, this._googleAuthDataSource, this._ref)
  ...
  Future<void> loginWithGoogle() async {
    ...
    try {
      ...
      final String tokenString = backendOk
          ? backendToken.toString()
          : 'google_local:${googleAccount.id}';

      final Map<String, dynamic> userDataForState =
          backendOk && backendResponse != null
      ...
      try {
        await _ref.read(googleLoginLocalStorageProvider).saveSession({
          ...
          'backendLoginResponse': backendResponse != null
              ? _jsonSafeDeepCopy(backendResponse)
              : null,
        });
      } catch (_) {}
